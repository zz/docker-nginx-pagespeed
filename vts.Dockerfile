...
RUN  useradd -r -s /usr/sbin/nologin nginx && mkdir -p /var/log/nginx /var/cache/nginx && \
	apt-get -y update && \
	apt-get -y --no-install-recommends install openssl ca-certificates curl wget git-core autoconf automake libtool libgeoip1 build-essential zlib1g-dev libpcre3-dev libxslt1-dev libxml2-dev libgd2-xpm-dev libgeoip-dev libgoogle-perftools-dev libperl-dev libgd3 unzip uuid-dev && \
    ...
    echo "Downloading VTS v${VTS_VERSION} from https://codeload.github.com/vozlt/nginx-module-vts/tar.gz/v${VTS_VERSION} ..." && wget --no-check-certificate -O - https://codeload.github.com/vozlt/nginx-module-vts/tar.gz/v${VTS_VERSION} --progress=bar --tries=3 | tar xzf  - -C /tmp && \
